...
.. Review `package.json` in `gremlin-javascript`, `gremlint`, and `gremlin-mcp`; the `package-lock.json` is now at the repository root — run `npm install` from there to regenerate it after version changes and review it
...
... Consider direct deploy with `npm publish --otp <otp>` from `gremlin-mcp/src/main/javascript/` - if using this method, ensure the `npm` version aligns with the one expected by the branch. Note: `gremlin-javascript` must be published before `gremlin-mcp` as it is a dependency.
